...
class NexysA7MIGIODDR(depth : BigInt) extends Bundle {
  require((depth<=0x8000000L),"NexysA7MIGIODDR supports upto 128 MB depth configuraton")
...
{
  require((depth<=0x8000000L),"NexysA7mig supports upto 128 MB depth configuraton")

  val io = IO(new NexysA7MIGIODDR(depth) with NexysA7MIGIOClocksReset {
    // User interface signals
