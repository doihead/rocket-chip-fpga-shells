package sifive.fpgashells.devices.xilinx.xilinxnexysa7mig
...
case class XilinxNexysA7MIGParams(
  address : Seq[AddressSet]
)

class XilinxNexysA7MIGPads(depth : BigInt) extends NexysA7MIGIODDR(depth) {
  def this(c : XilinxNexysA7MIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxNexysA7MIGIO(depth : BigInt) extends NexysA7MIGIODDR(depth) with NexysA7MIGIOClocksReset

class XilinxNexysA7MIGIsland(c : XilinxNexysA7MIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  require((depth<=0x8000000L),"Nexys4mig supports upto 128 MB depth configuraton")
  ...
  class Impl extends LazyRawModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxNexysA7MIGIO(depth)
    })
    ...
    val blackbox = withClockAndReset(childClock, childReset) { Module(new NexysA7mig(depth)) }
...
class XilinxNexysA7MIG(c : XilinxNexysA7MIGParams, crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxNexysA7MIGIsland(c, crossing))
